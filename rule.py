...
Description:
  Checks whether AWS DMS Endpoint has SSL mode set as require / verify-ca / verify-full.
...
Scenarios:
  Scenario: 1
    Given: No AWS DMS Endpoint exists
     Then: Return Empty list
  Scenario: 2
    Given: At least one AWS DMS Endpoint exists
      And: Configured KMS Key for encryption is managed by AWS
      Then: Return NON_COMPLIANT with annotation "This AWS DMS Endpoint is using AWS Managed Key for Encryption"
  Scenario: 3
    Given: At least one AWS DMS Endpoint exists
      And: Configured KMS Key for encryption is managed by Customer i.e. Standard Chartered
...
DEFAULT_RESOURCE_TYPE = 'AWS::DMS::Endpoint'
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    dms_endpoints = client.describe_endpoints()['Endpoints']
    evaluations = []
    ...
    for dms_endpoint_details in dms_endpoints:
        dms_kms_compliance = False
        kmsid = dms_endpoint_details['KmsKeyId']
        kms_description = kms_client.describe_key(KeyId=kmsid)
        
        if kms_description['KeyMetadata']['KeyManager']=='CUSTOMER':
            dms_kms_compliance = True
        
        if dms_kms_compliance is False:
        #SCENARIO 2
            evaluations.append(build_evaluation(dms_endpoint_details['EndpointIdentifier'], 'NON_COMPLIANT', event, annotation='This AWS DMS Endpoint is not using Customer Managed Key for Encryption'))
        else:
            #SCENARIO 3
            evaluations.append(build_evaluation(dms_endpoint_details['EndpointIdentifier'], 'COMPLIANT', event))
